...
const ConverterSelect = document.getElementById('unit-select');

const left = document.getElementById('left');
const right = document.getElementById('right');

const unitSelectLeft = left.querySelector('select');
const unitSelectright = right.querySelector('select');

window.onload = () => {
    fillOptions(ConverterSelect, property);
    let proIndex = ConverterSelect.selectedIndex;
    fillOptions(unitSelectLeft, unit[proIndex]);
    fillOptions(unitSelectright, unit[proIndex]);
}

const fillOptions = (id, menuArray) => {
    id.innerHTML = "";
    menuArray.forEach(element => {
        id.add(new Option(element, element));
    });
}
...
ConverterSelect.addEventListener('change', () => {
    console.log('changed');
    let proIndex = ConverterSelect.selectedIndex;
    const array = unit[proIndex]
    fillOptions(unitSelectLeft, array);
    fillOptions(unitSelectright, array);
})
...
const Calculate = (source, target) => {
    let sourceIP = parseFloat(source.querySelector('input').value);
    let proIndex = ConverterSelect.selectedIndex;

    let sourceIndex = source.querySelector('select').selectedIndex;
    let targetIndex = target.querySelector('select').selectedIndex;
    let sourceFactor = factor[proIndex][sourceIndex];
    let targetFactor = factor[proIndex][targetIndex];
    let result = sourceIP * sourceFactor;

    result = result / targetFactor;
    target.querySelector('input').value = result;
}


// For Dynamic Date
document.getElementById('copyright').innerText += new Date().toDateString();